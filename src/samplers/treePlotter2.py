...
class TreePlotter2(TreeSampler2):
    def __init__(self, space, R=128, auto=True, theta=1, theta_h=1):
        self.figure = plt.figure()
        self.ax = plt.axes()
        plt.ion()
        plt.show()
        super(TreePlotter2, self).__init__(space, R, auto, theta, theta_h)
        ...
        self.ax.add_patch(patches.Rectangle(xy=(self.h_region.low[0], self.h_region.low[1]),
                                            width=self.h_region.high[0] - self.h_region.low[0],
                                            height=self.h_region.high[1] - self.h_region.low[1],
                                            fill=True,
                                            edgecolor=None,
                                            alpha=0.2,
                                            color='green'))
