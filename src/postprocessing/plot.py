...
runs = glob.glob('../../log/cluster/2308/dqn_*/*/')
frames = []
...
print(df['num_run'].head)
# ys = ['agent', 'passenger', 'taxi']
# ys = ['light', 'sound', 'toy1', 'toy2']
ys = range(1)
y = ['R_{}'.format(i) for i in ys]
x = ['step']
params = ['--agent', '--self_imit', '--posInit', '--max_steps']
# df = df[(df['agent'] == 'dqng')]
df = df[(df['--posInit'] == 0)]
df = df[(df['--max_steps'] == 200000)]
...
# for i, (name, g) in enumerate(df.groupby(['--agent', '--self_imit', '--posInit'])):
